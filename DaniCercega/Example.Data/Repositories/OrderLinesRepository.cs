...
namespace Example.Data.Repositories
{
    public class OrderLinesRepository: IOrderLinesRepository
    {
        ...
        public TryAsync<List<Product>> TryGetExistingProducts(IEnumerable<int> ordersToCheck) => async () =>
        {
            var orders = await ordersContext.OrderLines
                                              .Where(orderLine => ordersToCheck.Contains(orderLine.OrderLineId))
                                              .AsNoTracking()
                                              .ToListAsync();
            return orders.Select(order => new Product(order.ProductId, order.Quantity)) // todo: get code from Product db
                           .ToList();
        };
    }
}
